...
The Mastra workflow intentionally exposes the deterministic recompute pipeline as one durable step. Candidate fan-out stays inside that step and is batched four cities at a time while the adapter's shared limiter enforces the provider-wide concurrency cap of six. Mastra `.foreach()` is not used because revision guards and the single 60-second deadline must cover the whole snapshot, and partial provider failures are aggregated into one canonical result. Splitting these calls into independently persisted steps would add snapshots without adding a recoverable boundary: every retry still has to revalidate the trip revision and adapter cache.
