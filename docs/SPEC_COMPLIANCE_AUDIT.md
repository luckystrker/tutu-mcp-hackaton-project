...
| §51 Security            | ✅      | HMAC-валидация initData (не initDataUnsafe), opaque invite токены (sha256), authz на каждом эндпоинте, organizer-only; приватные constraints фильтруются, кроме явно публичного города готового участника; ⚠️ suitability всегда "unknown" |
